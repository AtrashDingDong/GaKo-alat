...
//get path of extensions with: Platform.userExtensionDir;
//CLASSES - PSEUDO UGEN
...
GaKoSynth {
	...
	*new {
	...
	make {//INSTANCE method
		...
		//*******************************************************ARGS, VARS*****************************************************//
		arg nbNotes, nbInstr, height=800, envAtkTime=0.01, envRelTime=1.0, envLevel=1.0, envCurve=4;
...
//create a list of instr which will hold one dictionary per instrument
		...
		//Label
		StaticText(saihV, Rect((gap/4),(gap/4),100,20))
		.string_("Saih")
		...
		//Num boxes
		saihNumBoxes = List.fill(nbNotes, {arg i; NumberBox(saihV, Rect((gap/4)+(i*35),330,30,20))//-----------------------------------------------------ICI
			.value_(200)
			.clipLo_(20)
		...
		*/
		...
		//TODO: pouvoir choisir soi meme le numero de l'instrument (channel
		...
		//Label
		StaticText(window, Rect(xyOmbak[0] + (gap/4),xyOmbak[1]+(gap/4),100,20))
		.string_("Ombak")
		.font_(Font("Monaco", 18))
		.align_(\left)
		.background_(Color(1,1,1,0))
...
//note: valueAction doit etre apres .action pour marcher
		...
		.action_({
			arg obj;
			//obj.value_(ombakKnob.value);
			//ombakKnob.valueAction_(obj.value.explin(0,20,0,1))//the number box is passive, the slider is active
		//~saihMulSlider[i].valueAction_(obj.value.explin(100,4000,0,1));
		//obj.value.postln;
		//~saihSliders.valueAction_(obj.value);
		//obj.value.postln;
...
//-------------------------------start of add instr method------------
	...
	addInstr {
		...
		//Label
		label = StaticText(instrV, Rect(gap/4,gap/4,100,20))
		...
		//Ratio label
		ratioLabel = StaticText(instrV, Rect(gap/4,(gap+30+(gap/4)),100,20))
			.string_("Ratio")
		...
		//Dur label
		StaticText(instrV, Rect(gap/4,(gap+50+(gap/4)),100,20))
		.string_("Durasi")
		.font_(Font("Monaco", 14))
		.align_(\left)
		.background_(Color(1,1,1,0))
		...
		//Curve label
		curveLabel = StaticText(instrV, Rect(gap/4,(gap+70+(gap/4)),100,20))
		.string_("Curve")
		.font_(Font("Monaco", 14))
		.align_(\left)
		.background_(Color(1,1,1,0))
		...
		//Amp label
		ampLabel = StaticText(instrV, Rect((gap/4)+(400-5),(gap/2),100,20))
		.string_("Amp")
		.font_(Font("Monaco", 14))
		.align_(\left)
		.background_(Color(1,1,1,0))
